...
- `main` is green: CI, CodeQL, Trivy, and supply-chain workflows all passing.
- Semgrep Managed Scanning reports no new blocking findings on the Semgrep
  AppSec Platform.
...
| Branch protection on `main` | Require the `CI success` check | Single aggregate gate; see `ci.yml`. Requires a public repository or GitHub Pro |
| Branch protection on `main` | Require signed commits (recommended) | Author accountability |
| Discussions | Enabled | `SUPPORT.md` and the issue-template chooser link to it |
| Pull requests → Automatically delete head branches | Enabled | Keeps the branch list readable |
| Code security → Code scanning | Enabled | Receives CodeQL, Trivy, OSV, and Scorecard SARIF |
| Code security → Private vulnerability reporting | Enabled | The channel `SECURITY.md` points to. Not settable through the API on a private repository — enable it in the web UI, or after going public |
...
SARIF uploads need either a public repository or GitHub Advanced Security. Until
one of those holds, `.github/actions/code-scanning-available` detects the
condition and the scanners skip only their upload step; CodeQL, whose analysis is
inseparable from its upload, skips entirely. The scans that can run still gate
the build.
