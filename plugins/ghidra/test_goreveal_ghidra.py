...
class FakeGhidraAPI:
    ...
    def set_program_context(self, path, module_path, go_version):
    ...
    def rename_symbol(self, address, name):
...
class GoRevealGhidraAdapterTests(unittest.TestCase):
    ...
    def test_apply_actions_uses_fake_api(self):
        actions = [
            {"kind": "program", "path": "/tmp/sample.bin", "module_path": "example.com/sample", "go_version": "go1.26.1"},
        ...
        self.assertIn(("set_program_context", "/tmp/sample.bin", "example.com/sample", "go1.26.1"), api.calls)
    ...
    def test_fixture_export_builds_actionable_payload(self):
        ...
        self.assertTrue(any(action["kind"] == "program" and action["module_path"] == "example.com/gorevealfixture" for action in actions))
        self.assertTrue(any(action["kind"] == "symbol" and action["name"] == "main.main" for action in actions))
