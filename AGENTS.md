...
The active execution authority is the RT1 product design and Horizon A plan
listed above. `RT1-S0` is the first implementation lane; do not resume an old
numbered sprint merely because an older document still calls it active.

The following is a preserved capability and compatibility snapshot. It is
useful context, but it does not override RT1 sprint ordering:
- architecture docs
- plan docs
- baseline inventory
- agent docs and skills
- Podman-first development environment
- native capability transfer completed in Sprint 11:
...
- Sprint 11/12 constraints that remain valid when RT1 touches these surfaces:
  ...
  - the stripped ELF checkpoint remains part of the preserved Sprint 12 compatibility contract:
  ...
  - use `docs/plans/2026-03-20-goreveal-functional-assessment.md` as a historical product snapshot, not the active queue
